...
for device_index, (device, data_adc, data_t2, data_z) in \
    ...
    with tf.device(device):
        ...
        # l1 loss
        l1_adc_loss = tf.reduce_mean(tf.abs(real_data_adc - fake_data_adc))
        l1_t2_loss = tf.reduce_mean(tf.abs(real_data_t2 - fake_data_t2))
        ...
        tf.summary.image("Real to ADC", fake_data_adc, max_outputs=5)
        tf.summary.image("Real to T2", fake_data_t2, max_outputs=5)
...
g1_var = [t for t in t_vars if 'sl' in t.name] + [t for t in t_vars if 'Generator_ADC' in t.name] + [t for t in t_vars if 'Encoder' in t.name]
g2_var = [t for t in t_vars if 'sl' in t.name] + [t for t in t_vars if 'Generator_T2' in t.name]
...
def form_results():
    ...
    """
    folder_name = "/{0}".format(datetime.datetime.now().strftime('%Y-%m-%d__%H:%M:%S'))
